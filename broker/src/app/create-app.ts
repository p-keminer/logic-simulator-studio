...
const BROKER_SERVICE_ID = 'logic-simulator-broker';
...
export const createApp = (
  ...
  const sessionService =
    ...
    (() => {
      const retentionMs =
        config.sessionStoreInactiveRetentionSeconds * 1_000;

      return new DefaultSessionService(
        new InMemorySessionStore({
          inactiveRetentionMs: retentionMs,
          maxRecords: config.sessionStoreMaxRecords,
        }),
        new InMemoryKeyReferenceStore({
          maxRecords: config.sessionStoreMaxRecords,
          revokedRetentionMs: retentionMs,
        }),
  ...
  const conversationHistoryStore =
    ...
    new InMemoryConversationHistoryStore({
      idleTtlMs: config.conversationIdleTtlSeconds * 1_000,
      maxRecords: config.conversationStoreMaxRecords,
      maxStoredBytesPerConversation: config.conversationMaxStoredBytes,
      maxStoredTurnsPerConversation: 32,
    });
  const auditSink =
    options.auditSink ??
    new InMemoryAuditSink({
      maxEvents: config.auditStoreMaxEvents,
      retentionMs: config.auditStoreRetentionSeconds * 1_000,
    });
  const metricsSink =
    options.metricsSink ??
    new InMemoryMetricsSink({
      maxSamples: config.metricsStoreMaxSamples,
      maxSeries: config.metricsStoreMaxSeries,
      sampleRetentionMs: config.metricsSampleRetentionSeconds * 1_000,
    });
