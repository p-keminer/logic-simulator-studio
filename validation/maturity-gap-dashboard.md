# Logic Gate Simulator â€” Maturity Gap Dashboard
...
**Last updated 2026-03-07 (post-P0):** P0 (Z/X signal model + multi-driver bus resolution), P1a (VHDL `&` operator fix), P1b (/OE defaultInputValues), and P1-1 (74HC373 Verilator LATCH â€” Verilog-2001 + `/* verilator lint_off LATCH */`â€¦`/* verilator lint_on LATCH */`) are now resolved. Test suite: **713/713**. Focused-nine: **12/12 PASS, 0 tooling warnings**. No hard P0 or P1-1 blockers remain.

---

## Criterion 1 â€” Formal Simulation Model
...
- Z propagates correctly to downstream gate inputs â€” downstream NOT(Z) produces X [CONFIRMED: `tri_not_sanitized` PASS]
...
|---|---|---|
| ~~**HI_Z (Z=2) is sanitized to 0 before downstream evaluate()**~~ | **RESOLVED P0 2026-03-07** â€” `tri_not_sanitized` PASS, triY=2, invOut=3 | ~~P0 blocker~~ done |
| No setup/hold violation modeling â€” metastability produces 0 not X | [INFERRED from type definition] | model-limit |
| `prevClk` stored in customState but absent from `stateKeys` â€” hidden simulator state | gate-gap-analysis.md Â§6 | core-risk |
| MS_JK_FF: master state `qM` not in stateKeys â€” partially observable only | gate-gap-analysis.md Â§6 | core-risk |
| BIN_CTR7S/BIN_CTR_99: dual-write of `count` + `cnt0..cnt3` â€” inconsistency risk | gate-gap-analysis.md Â§6 | core-risk |
...
**Priority: P1** (hidden state keys â€” P0 Z sanitization resolved 2026-03-07)

---

## Criterion 2 â€” Cross-Layer Consistency
...
- 74HC74 contract documents the /PRE=/CLR=0 forbidden state and the Q=1, Qn=1 output â€” consistent with source
...
| Issue | Evidence | Severity |
|---|---|---|
| ~~**Multi-driver HDL export generates duplicate port declarations**~~ | **RESOLVED P0 2026-03-07** â€” `multi_driver_same_input` all tools PASS, conflict resolves to X | ~~P0 blocker~~ done |
| ~~VHDL `&` operator on STD_LOGIC is ambiguous~~ | **RESOLVED P1a 2026-03-07** | ~~P0~~ done |
| ~~**74HC373 Verilog always@(*) latch triggers Verilator LATCH warning-as-error**~~ | **RESOLVED P1-1 2026-03-07** â€” Verilog-2001 export with `/* verilator lint_off LATCH */`â€¦`/* verilator lint_on LATCH */` inline comments; all tools PASS | ~~P1~~ done |
| MS_JK_FF HDL export uses `negedge clk` only â€” does not model master-slave behavior | gate-gap-analysis.md Â§4.2 | synthesis-risk |
| D_LATCH Verilog `always @(*)` causes latch inference warnings in synthesis tools | gate-gap-analysis.md Â§4.4 | synthesis-risk |
| ~~17 basic gates (AND/OR/NOT/NAND/NOR/XOR/XNOR/BUFFER + multi-input variants) have no toVerilog/toVHDL~~ | **RESOLVED P0 2026-03-07** â€” all logic_basic and logic_multi gates now export | ~~P0 blocker~~ done |
| AND_C/OR_C/XOR_C output naming (`q`, `q_n`) inverted vs NAND_C/NOR_C â€” inconsistency | gate-gap-analysis.md Â§7.3 | documentation-gap |
...
- **RESOLVED P0 2026-03-07:** Basic gate HDL (AND/OR/NOT/NAND/NOR/XOR/XNOR etc.) â€” toVerilog/toVHDL added for all logic_basic and logic_multi gates.

### Next Steps

1. ~~Fix 74HC373 Verilog Verilator LATCH~~ â€” **RESOLVED P1-1 2026-03-07** (Verilog-2001 + lint_off/lint_on)
2. Document MS_JK_FF HDL approximation as synthesis-risk in the gate contract

**Priority: P2** (MS_JK_FF approximation â€” P0 and P1-1 items resolved 2026-03-07)

---

## Criterion 3 â€” Verification Strength
...
- Focused-nine audit covers 12 high-risk cases â€” all now pass [CONFIRMED: focused-nine-summary.json]
- Risk classes are assigned per-gate, enabling risk-weighted test prioritization
- Multi-driver conflict behavior is now defined and verified: conflicting drivers â†’ X (3) [CONFIRMED]

### Confirmed Weaknesses

| Issue | Evidence | Severity |
|---|---|---|
| No automated truth-table-exhaustive test runner for any combinational gate | [INFERRED â€” no test directory found] | P1 |
...
| 26 documentation-gap entries â€” gate behavior partially undocumented | gate-gap-analysis.md Â§10 | P2 |
...
**Priority: P1** (truth-table runner, step-sequence runner â€” P0 multi-driver definition resolved 2026-03-07), **P2** (contract expansion)

---

## Criterion 4 â€” External Reference Toolchain
...
All 12 focused-nine cases pass all tools without errors or warnings. 74HC373 Verilog latch warning resolved via Verilog-2001 export with `/* verilator lint_off LATCH */`â€¦`/* verilator lint_on LATCH */` inline pragmas (P1-1, 2026-03-07).
...
| Test Case | Tool | Fix |
|---|---|---|
| `multi_driver_same_input` | iverilog, verilator, ghdl | Duplicate port declarations fixed â€” all tools now PASS (P0 2026-03-07) |
| `hc194_modes` | ghdl | VHDL `&` cast fix (P1a 2026-03-07) â€” now PASS |
| `mixed_datapath` (ALU4) | ghdl | Same fix (P1a 2026-03-07) â€” now PASS |
| `hc373_oe_z` | verilator | `%Warning-LATCH` resolved: Verilog-2001 + `/* verilator lint_off LATCH */`â€¦`/* verilator lint_on LATCH */` (P1-1 2026-03-07) |
...
1. ~~Fix 74HC373 Verilog latch~~ â€” **RESOLVED P1-1 2026-03-07** (Verilog-2001 + `/* verilator lint_off LATCH */`â€¦`/* verilator lint_on LATCH */`)
2. Extend CI beyond focused-nine core: golden-corpus regression and, later, UI timing audit

**Priority: P2** (broader regression automation â€” all tooling warnings resolved 2026-03-07)

---

## Criterion 5 â€” Hierarchy and Large Design Support
...
Custom ICs exist (`category=custom`) but were explicitly excluded from the gate contract analysis â€” they are dynamically registered at runtime and cannot be statically verified. No evidence of hierarchy-specific test coverage exists. Multi-driver bus conflict behavior is now correctly modeled.
...
- Multi-driver conflict on a shared net now correctly produces X â€” bus topologies are no longer silently wrong [CONFIRMED: focused-nine-summary.json]

### Confirmed Weaknesses

| Issue | Evidence | Severity |
|---|---|---|
| Custom ICs: no contracts, no static analysis possible | gate-gap-analysis.md Â§8 | P2 |
| Golden-Corpus v1 exists, but there is still no large (>20 gates) or hierarchical regression case in active use | `golden-corpus-v1.md` â€” 12 reference circuits, none large/hierarchical; custom ICs still uncovered | P1 |
| ~~STT variable limit (max 8) blocks state inspection for all ICs with >8 inputs+state~~ | **PARTIALLY RESOLVED (post-P0):** STT now renders a "Reduzierte Ansicht" (reduced view) for wide sequentials â€” 8â€“64 rows of meaningful data instead of error message | P2 (residual) |
...
- **PARTIALLY RESOLVED (post-P0):** STT reduced view â€” wide sequential ICs (74HC373, 74HC374, 74HC595, 74HC161, 74HC163, 74HC194, mixed_datapath) now show a partial but useful state table, confirmed in focused-nine UI audit. [CONFIRMED: focused-nine-ui-summary.json 2026-03-07]
...
## Criterion 6 â€” Formal Quality Assurance
...
- vitest: 713/713 pass â€” test suite is comprehensive for simulation correctness

### Confirmed Weaknesses

| Issue | Evidence | Severity |
|---|---|---|
| focused-nine core is automated in CI, but UI timing audit and golden-corpus regression are not yet integrated | `.github/workflows/quality-gates.yml` â€” `focused-nine-core` present, UI/golden-corpus still manual | P1 |
...
## Criterion 7 â€” Technical Depth

**Objective:** The simulator should model the electronics faithfully enough to be useful for learning and circuit design â€” not just as a visual toy.
...
- Four-valued logic (0/1/Z/X) is architecturally complete â€” Z propagates, conflicts yield X [CONFIRMED]
...
| Issue | Evidence | Severity |
|---|---|---|
| SCHMITT trigger has no hysteresis â€” modeled as inverting buffer only | gate-gap-analysis.md Â§7.3 | model-limit |
| No metastability, setup/hold violation modeling â€” violations produce 0, not X | [INFERRED â€” no X state for timing violations] | model-limit |
| SR_LATCH S=R=1 â†’ Q=0, Qn=0 (both low) â€” real hardware is undefined/race | gate-gap-analysis.md Â§4.1 | model-limit |
| 74HC74 /PRE=/CLR=0 â†’ Q=1, Qn=1 (both high) â€” real hardware is undefined | gate-gap-analysis.md Â§4.1 | model-limit |
| ~~TRIBUF, 74HC373, 74HC374, 74HC595 have unsafe /OE defaults~~ | **RESOLVED P1b 2026-03-07** | ~~core-risk~~ done |
| ~~Z propagation broken â€” bus architectures produce wrong results~~ | **RESOLVED P0 2026-03-07** â€” Z propagates; NOT(Z)â†’X confirmed | ~~P0~~ done |
| ~~17 basic gates have no HDL export â€” cannot synthesize any real circuit~~ | **RESOLVED P0 2026-03-07** â€” toVerilog/toVHDL added to all logic_basic and logic_multi | ~~P0~~ done |
...
~~Fix /OE defaultInputValues~~ â€” RESOLVED P1b 2026-03-07
~~Fix Z propagation~~ â€” RESOLVED P0 2026-03-07
~~Add basic gate HDL export~~ â€” RESOLVED P0 2026-03-07

**Priority: P2** (model-limit docs â€” all P0 items resolved 2026-03-07)
...
| ~~P0-3~~ | VHDL `&` operator on STD_LOGIC â€” GHDL parse failure (74HC194, ALU4) | **DONE P1a 2026-03-07** |
| ~~P0-4~~ | 17 basic gates missing toVerilog/toVHDL â€” no real circuit can be exported | **DONE P0 2026-03-07** |
| ~~P1-2~~ | Unsafe /OE defaultInputValues on TRIBUF, 74HC373, 74HC374, 74HC595 | **DONE P1b 2026-03-07** |
| ~~P1-1~~ | 74HC373 Verilog latch â€” Verilator LATCH warning-as-error | **DONE P1-1 2026-03-07** â€” Verilog-2001 + `/* verilator lint_off LATCH */`â€¦`/* verilator lint_on LATCH */` |

### P1 â€” High Priority (fix before production use)

| ID | Issue | Criterion |
|---|---|---|
| P1-3 | prevClk hidden state not in stateKeys (all edge-triggered FFs) | Crit 1, Crit 2 |
| ~~P1-4~~ | ~~STT variable limit blocks UI verification for all ICs~~ | **PARTIALLY RESOLVED (post-P0):** Reduced-view renders 8â€“64 rows for wide ICs â€” remaining gap is full enumeration |
...
### P2 â€” Medium Priority (completeness and polish)
...
| P2-3 | Custom IC (category=custom) contracts â€” no static verification strategy | Crit 5 |
...
| `validation/focused-nine-ui-summary.json` | UI and STT audit results (12/12 smoke PASS, 5 semantic WARN â€” headless RAF limit, steps=0) |
