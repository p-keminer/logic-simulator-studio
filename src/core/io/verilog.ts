...
import { flattenCustomICs } from './flattenCustomIC';
...
function renderVerilogExtraRegs(
  ...
  for (const { name, width, depth, initData } of regs) {
    if (depth !== undefined) {
      ...
      const zeroWord = '0'.repeat(nibbles);
      lines.push(`  reg [${width - 1}:0] ${name} [0:${depth - 1}];`);
      if (initData) {
        const idxName = `${name}_init_idx`;
        lines.push(`  integer ${idxName};`);
        lines.push(`  initial begin`);
        lines.push(`    for (${idxName} = 0; ${idxName} < ${depth}; ${idxName} = ${idxName} + 1) begin`);
        lines.push(`      ${name}[${idxName}] = ${width}'h${zeroWord};`);
        lines.push(`    end`);
...
function renderBlockedVerilog(moduleName: string, reason: string): string {
  return [
    `// Generated by LogicSim`,
    `// HDL EXPORT BLOCKED — ${reason}`,
...
export function generateVerilog(circuit: Circuit): string {
  ...
  try {
    ...
    return renderBlockedVerilog(moduleName, String(error).slice(0, 200));
  }
  const { byPort } = buildWireMap(exportCircuit);
  ...
  for (const wire of Object.values(exportCircuit.wires)) {
    const srcGate = exportCircuit.gates[wire.from.gateId];
    if (!srcGate) continue;
