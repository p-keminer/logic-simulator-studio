...
function renderBlockedVHDL(entityName: string, reason: string): string {
  return [
...
export function generateVHDL(circuit: Circuit): string {
  ...
  try {
    ...
    return renderBlockedVHDL(entityName, String(error).slice(0, 200));
  }
  const byPort = buildPortMap(exportCircuit);
