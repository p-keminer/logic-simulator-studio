#!/usr/bin/env bash
# Linux: Einmalig ausfuehrbar machen: chmod +x start-launcher.sh
# Danach Doppelklick im Dateimanager (als Programm ausfuehren).
...
if ! command -v node &>/dev/null; then
  ...
  echo "  oder:  sudo apt install nodejs npm   (Debian/Ubuntu)"
  echo "  oder:  nvm install --lts             (nvm)"
